...
class RotationTuning(QWidget):
    def __init__(self):
        ...
        self.cam1_trans_x          = 0.8571999277380136 # -1.4m to be at wheelbase center
        self.cam1_trans_y          = -0.1526472937107812
        self.cam1_trans_z          = 1.623767855306573  # already + 0.362 [wheel radius]
        self.cam1_translation      = np.array([self.cam1_trans_x, self.cam1_trans_y, self.cam1_trans_z])        
        ...
        self.cam2_roll          = self.cam1_roll  #-1.58159
        self.cam2_pitch         = self.cam1_pitch # 0.03591
        self.cam2_yaw           = self.cam1_yaw   #-1.59909
        self.cam2_trans_x       = self.cam1_trans_x #0.8571999277380136 # -1.4m to be at wheelbase center
        self.cam2_trans_y       = self.cam1_trans_y #-0.1526472937107812
        self.cam2_trans_z       = self.cam1_trans_z #1.623767855306573  # already + 0.362 [wheel radius]        
        self.cam2_translation   = self.cam1_translation
        ...
        for i in range(self.cam1_impoints.shape[0]):
            ...
            homopoint_cam1coordinates \
                                = np.matmul(self.K_inverse, img_point.T)*homo_scale
    ...
    #           scroller's value  
        ...
        self.label1 = QLabel('roll X-ego 3rd-rotation --pitch effect | ' + str(round((self.cam2_roll)/np.pi*180, 2)) + ' degree', self)
        ...
        self.label2 = QLabel('pitch Y-ego 2nd-rotation --roll effect | ' + str(round((self.cam2_pitch)/np.pi*180, 2)) + ' degree', self)
        ...
        self.label3 = QLabel('yaw Z-ego 1st-rotation --yaw effect | ' + str(round((self.cam2_yaw)/np.pi*180, 2)) + ' degree', self)
        ...
        # # translations
            # from {-1 .. +1}, scale factor of 400 for scroller,
            # so setRange(0, 3*2*400) and setValue(int((self.cam2_trans_x + 1)*400))
        """# x"""
        sld4 = QScrollBar(Qt.Horizontal, self)
        sld4.setRange(0, 3*2*400)
        sld4.setValue(int((self.cam2_trans_x + 3)*400))
        sld4.setFocusPolicy(Qt.NoFocus)
        sld4.setPageStep(1)
        sld4.valueChanged.connect(self.updateLabel_trans_x)
        self.label4 = QLabel('trans_x | ' + str(round((self.cam2_trans_x), 2)) + ' m', self)
        ...
        """# Y"""
        sld5 = QScrollBar(Qt.Horizontal, self)
        sld5.setRange(0, 3*2*400)
        sld5.setValue(int((self.cam2_trans_y + 3)*400))
        sld5.setFocusPolicy(Qt.NoFocus)
        sld5.setPageStep(1)
        sld5.valueChanged.connect(self.updateLabel_trans_y)
        self.label5 = QLabel('trans_y | ' + str(round((self.cam2_trans_y), 2)) + ' m', self)
        ...
        """# Z"""
        sld6 = QScrollBar(Qt.Horizontal, self)
        sld6.setRange(0, 3*2*400)
        sld6.setValue(int((self.cam2_trans_z + 3)*400))
        sld6.setFocusPolicy(Qt.NoFocus)
        sld6.setPageStep(1)
        sld6.valueChanged.connect(self.updateLabel_trans_z)
        self.label6 = QLabel('trans_z | ' + str(round((self.cam2_trans_z), 2)) + ' m', self)
    ...
    def updateLabelYaw(self, value):
        ...
        # from trans_ is in {-1 .. +1}, scale factor of 400 for scroller, so, value/400.0 - 1
    def updateLabel_trans_x(self, value):
        self.label4.setText('trans_x | ' + str(round((value/400.0 - 3), 2)) + ' m')
        self.cam2_trans_x = value/400.0 - 3
        self.visualizeProjection()

    def updateLabel_trans_y(self, value):
        self.label5.setText('trans_y | ' + str(round((value/400.0 - 3), 2)) + ' m')
        self.cam2_trans_y = value/400.0 - 3
        self.visualizeProjection()
        
    def updateLabel_trans_z(self, value):
        self.label6.setText('trans_z | ' + str(round((value/400.0 - 3), 2)) + ' m')
        self.cam2_trans_z = value/400.0 - 3
        self.visualizeProjection()
    ...
    def visualizeProjection(self):
        ...
        self.cam2_vehicle[:3,:3] \
                              = (R.from_euler('ZYX', [self.cam2_yaw, self.cam2_pitch, self.cam2_roll])).as_matrix()                              
        self.cam2_translation = np.array([self.cam2_trans_x, self.cam2_trans_y, self.cam2_trans_z])                                    
        self.cam2_vehicle[:3, 3] \
                              = self.cam2_translation
        ...
        self.cam2_impoints                  = np.empty((0,2)).astype(np.float32)
        ...
        for i in range(self.homopoints_egocoordinates_4x_.shape[0]):
            ...
            # homo2_cam2coordinates_3x1 = _cam2coordinates_3x1
            ...
            img_point_cam2      = (np.matmul(self.mtx, _cam2coordinates_3x1.T)) #"""#.astype(np.int32) ???""" 
                ...
                # if homo point is not [x y 1 1] (z#1), in case we use homo_scale # 1
                # # then img_point is [u v w] different than [u v 1], we need to make w ->> 1
                
            #***
            _u                  = img_point_cam2[0]/img_point_cam2[2]
            _v                  = img_point_cam2[1]/img_point_cam2[2]
            img_point_cam2[0]   = _u
            img_point_cam2[1]   = _v
            img_point_cam2[2]   = 1
        ...
        self.cam1_srcpoints     = [tuple(x.astype(np.float32)) for x in self.cam1_impoints[:4]] # 4 sources point for image warping
        ...
        for i in range(len(self.cam1_srcpoints)):
            if self.cam1_srcpoints[i][0] > 0 and self.cam1_srcpoints[i][1] > 0:
                ...
                cv2.circle(image_cp, tuple(np.int32(self.cam1_srcpoints[i])), 4, colori, -1)
        ...
        self.cam2_dstpoints     = [tuple(x.astype(np.float32)) for x in self.cam2_impoints[:4]] # 4 sources point for image warping
